      ...
      <div className="row between center wrap gap12">
        {showTitle ? (
          <div className="col gap4">
            ...
            <div className="txt">기간: {esgReport.period}</div>
          </div>
        ) : (
          <div />
        )}
        <div className="row gap8">
          <button type="button" className="wf-btn accent" onClick={() => alert('데모: PDF 리포트를 다운로드합니다.')}>
            PDF 다운로드
          </button>
          <button type="button" className="wf-btn" onClick={() => alert('데모: 브랜딩 자료를 다운로드합니다.')}>
            브랜딩 자료 다운로드
          </button>
        </div>
      </div>

      <StatRow stats={esgReport.stats} />
      ...
      <div className="wf-card pad24 col gap16">
        {esgReport.sections.map((s) => (
          <div className="col gap8" key={s.title}>
            ...
            <div className="wf-box pad16 txt">{s.body}</div>
          </div>
        ))}
        <div className="h3">4. 환경 임팩트 결과</div>
