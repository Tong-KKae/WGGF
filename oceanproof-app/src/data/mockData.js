...
export const esgReport = {
  ...
  sections: [
    { title: '1. 프로젝트 개요', body: '2026년 2분기 동안 GREENERS 주식회사는 고성 잘피밭 복원, 부산 해안 쓰레기 정화, 여수 갯벌 모니터링 등 국내 3개 해역에서 진행된 시민 참여형 해양 복원 프로젝트를 후원했습니다. 총 680명의 시민이 참여했으며, 위성·드론·GIS 기반 MRV(측정·보고·검증) 체계를 통해 모든 활동 데이터의 신뢰성을 확보했습니다.' },
    { title: '2. 데이터 수집 방식', body: '참여 시민은 현장 QR코드를 스캔해 별도 앱 설치 없이 활동을 기록합니다. 촬영한 사진에는 GPS 좌표와 타임스탬프가 자동으로 태깅되며, 이 원시 데이터는 검증 전 1차 입력값으로 처리되어 이후 위성·드론 조사와 교차 검증됩니다.' },
    { title: '3. MRV 검증 절차', body: '시민 입력 → 위성 영상 기반 공간 변화 탐지 → 드론 고해상도 샘플링 → 공공 GIS 데이터셋 교차검증 → 최종 데이터셋 확정, 총 5단계 절차를 거쳐 데이터를 검증합니다. 각 단계를 통과한 데이터만 최종 ESG 성과 지표에 반영됩니다.' },
    { title: '5. ESG 커뮤니케이션 활용 문구', body: '당사는 2026년 2분기 OceanProof 플랫폼을 통해 고성·부산·여수 해역의 해양 생태계 복원 활동에 680명의 시민과 함께 참여했으며, 위성·드론·GIS 기반의 독립적 검증을 거쳐 8.4ha의 해양 생태계 복원과 3.6tCO₂의 탄소 격리 효과를 확인했습니다.' },
  ],
